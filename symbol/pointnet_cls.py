...
def pointnet(num_classes, batch_size, num_points, bn_mom=0.9, workspace=512, scope="pointnet_"):
    point_cloud = mx.sym.Variable(name='data')
    transform = input_transform_net(point_cloud, batch_size, num_points, workspace, bn_mom, scope=scope + "itn_")
    point_cloud_transformed = mx.sym.batch_dot(point_cloud, transform, name=scope + "input_transform")
    input_image = mx.sym.expand_dims(point_cloud_transformed, axis=1)
    ...
    transform = feature_transform_net(conv1, batch_size, num_points, workspace, bn_mom, scope=scope + "ftn_")
    conv1_reshaped = mx.sym.Reshape(conv1, (-1, num_points, 64), name=scope + "conv1_reshape")
    ...
    mx.sym.make_loss(name='transform_mat_loss', data=mat_diff_loss, grad_scale=1.0 / (batch_size*2.0))
